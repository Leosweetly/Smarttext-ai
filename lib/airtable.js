...
export async function getBusinessById(id) {
  try {
    // Check cache first
    const cacheKey = `business:id:${id}`;
    if (businessCache.has(cacheKey)) {
      console.log(`🔍 Business found in cache by ID: ${id}`);
...
export async function getBusinessByPhoneNumber(phoneNumber) {
  try {
    // Check cache first
    const cacheKey = `business:phone:${phoneNumber}`;
    if (businessCache.has(cacheKey)) {
      console.log(`🔍 Business found in cache by phone number: ${phoneNumber}`);
      return businessCache.get(cacheKey);
    }

    console.log(`🔍 Business not in cache, fetching from Airtable: ${phoneNumber}`);
    const businessesTable = getTable("Businesses");
    const records = await businessesTable.select({
      view: "Grid view",
    ...
    // Store in cache
    businessCache.set(cacheKey, business);
    console.log(`✅ Business cached with key: ${cacheKey}`);
...
export async function updateBusiness(id, data) {
  try {
    ...
    // Invalidate cache entries for this business
    const cacheKey = `business:id:${id}`;
    businessCache.delete(cacheKey);
    
    // If phone number is updated, also invalidate that cache entry
    if (data.phoneNumber) {
      const phoneCacheKey = `business:phone:${data.phoneNumber}`;
      businessCache.delete(phoneCacheKey);
    }
