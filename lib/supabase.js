...
/**
 * Get the client-side Supabase client (uses anon key)
...
function getSupabaseClient() {
  ...
  try {
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      console.warn('Missing Supabase client-side environment variables');
      return null;
    }
...
function getSupabaseServerClient() {
  ...
  try {
    ...
    console.error('Error initializing server-side Supabase client:', error);
    return null;
  }
}

// Export both client instances
...
// Export server-side client for API routes
export const supabaseServer = {
  from: (table) => {
    const client = getSupabaseServerClient();
    if (!client) {
      throw new Error('Supabase server client not initialized. Check environment variables.');
    }
    return client.from(table);
  }
};
...
export async function getBusinessByPhoneNumberSupabase(phoneNumber) {
  try {
    ...
    if (data && data.length > 0) {
      ...
      console.log('[business lookup] Business data:', business);
      return business;
    } else {
